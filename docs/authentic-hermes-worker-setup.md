...
4. creates one exclusive private config in the ephemeral home with `agent.max_turns: 8`, then invokes the authentic CLI from that empty home/working directory with `chat --query <prompt> --quiet --max-turns <bounded> --provider <configured> --model <configured> --ignore-rules --toolsets search`;
