...
- reviewed Qwen3 1.7B model identity `8f68893c685c`.

The manual workflow needs no founder-owned provider credential. It installs a checksum-verified Ollama release on the disposable GitHub runner, pulls the reviewed `qwen3:1.7b` artifact, verifies its published model identity and tool capability, and exposes it only on loopback through Hermes's `custom` provider boundary. This smaller tool-capable model is selected so the bounded live proof can finish on a GitHub CPU runner; it does not change Kernel authority or acceptance thresholds. The model and runtime disappear with the runner. The workflow still publishes only `.hephaestus/live-acceptance-report.json` after local redaction. A workflow definition or skipped/blocked run is not proof: acceptance requires the report itself to show all L1→L7 checks green on the tested SHA.
