...
- Hermes automatic discovery is certified read-only and uses the equivalent of `--safe-mode --toolsets search -z`; an incompatible Hermes runtime is visibly blocked rather than falling back to broad tools;
