...
- Channel: internal
- Candidate: `0.1.0-internal.8`
- Windows entrypoint: `Install Efesto.cmd`
- Public launch approved: **no**
- `0.1.0-internal.7` is frozen as a failed automated-qualification candidate and must not be reused.
...
2. Use only the `efesto-v0.1.0-internal.8-windows.zip` artifact from the successful `Internal Test Package` workflow on `main` after both packaged-install qualification jobs pass.
