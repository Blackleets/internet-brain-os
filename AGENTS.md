...
- Hephaestus is the provider-neutral knowledge-forging and memory-authority kernel.
- Agents are external execution principals, not the Kernel aggregate root and never the authority over their own history.
...
- Protected Kernel authority modules must pass `pnpm architecture:check`.
- Private chain-of-thought is not Evidence, a Claim, forensic causality, or durable Memory merely because an agent emits it.
- Forensic causality must come from persisted observable relationships and Kernel decisions, not inferred hidden intent.
- Event-sourced history is selective: use additive history where replay, provenance, authority, lifecycle, or security requires it; do not event-source every implementation detail.
- Do not add infrastructure because it looks enterprise-grade; require a measured product, reliability, security, scale, or operability need.
