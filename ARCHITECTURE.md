...
Conversation transport supports incremental NDJSON streaming with caller
cancellation. Ollama NDJSON and OpenAI-compatible SSE are translated behind the
same local adapter boundary. Completed exchanges are stored in a bounded,
owner-private `chat-conversations.json` file and may optionally retain a Case
...
Product-value measurement remains local-first. At first Kernel start, the knowledge store initializes one private `local_installation` cohort containing only a schema version, unit and start timestamp—no global user/device identifier. Trusted Goal authorization receipts provide the activation boundary. Once activated, a second distinct authorized Goal provides the repeat-usage observation. The resulting `0/1` or `1/1` ratios describe only that installation and must never be presented as a population rate. Missing or malformed cohort metadata fails measurement closed, and no scorecard read uploads telemetry or grants execution authority.

A local mission worker connects the leased transport to an explicitly configured Hermes adapter executable. The adapter is spawned without a shell, receives only the authorized Goal mission as bounded JSON on standard input, and returns at most 20 findings as JSON on standard output. It prefers canonical directly readable public pages and avoids login/paywall/redirect/search/JavaScript shells, improving verification yield without turning search snippets into Evidence. The preferred model response is strict URL-only JSON; if the final response is prose or malformed JSON, the adapter may discard every non-URL byte and translate only deduplicated literal HTTP(S) URLs into neutral unverified candidates. Valid structured output with an invalid schema or unsupported authority fields remains rejected, and every fallback URL still passes the Kernel's credential/private-address rejection, `web.read`, Evidence and classification gates. Kernel verification retains and hashes the complete fetched body as Evidence, while the page-context view used for classification and Opportunity projection is deterministically capped at the shared 12,000-character validation boundary; this bounded view never replaces or weakens source provenance. Every authentic invocation gets a fresh `HERMES_HOME` and working directory, creates the only profile config there exclusively with an at-most-eight-turn limit and the already-configured provider/model route, ignores rules/memory, refuses inherited project plugins, disables private-URL search support, and exposes only Hermes' official `search` toolset. A run may select a stricter cap but cannot expand it beyond eight. Hermes v0.20 `--safe-mode` is intentionally not used here because it also disables the bundled backend plugins that provide public web search; `--ignore-user-config` is also not used because its built-in profile restores a 500-turn default. The scripted `-z` path is not used because that pinned implementation fails to pass `agent.max_turns` into `AIAgent`. Isolation and the actual loop bound instead come from the exclusive private profile plus `chat --query <prompt> --quiet --max-turns <bounded> --provider <configured> --model <configured> --ignore-rules --toolsets search`, the ephemeral home/cwd, and the Kernel's independent URL/fetch/authority gates. Provider/model route values are inherited from the already-configured process environment, length/control-character checked, mirrored into the isolated profile to satisfy Hermes's pre-agent startup guard, and passed as direct spawn arguments without a shell. Live execution budgets must be strictly nested as adapter deadline < worker deadline < terminal-observation deadline < outer job deadline. Timeout or output-limit termination does not settle until the adapter process has closed, with bounded forced termination when graceful shutdown is ignored. Normal one-click recovery remains capped at three attempts; isolated acceptance may select a stricter fail-closed attempt count from one through three. Efesto retains all validation and memory authority. The extension derives honest Agent Hub display states from persisted missions; those states are also the future animation contract for the pixel-forge UI. No Hermes executable is bundled and synthetic worker tests do not satisfy the authentic-runtime acceptance proof.

Efesto bootstrap is a shared local readiness contract, not a second product surface. The Windows launcher, extension, and future Efesto browser shell consume `efesto.bootstrap-status.v1`, which reports Kernel, Hermes, Obsidian, pairing, overall readiness, safe user-facing copy, technical diagnostics, and recoverable actions without exposing tokens or secrets. The Kernel `/bootstrap/status` route uses the same probe module as the launcher instead of server-local simulated readiness. The one-click Kernel and worker doctor share fail-closed Hermes executable detection: an explicit executable, the standard Windows installation, or a real `PATH` entry may activate the bundled adapter, while a missing executable remains unavailable. The doctor may read the already-persisted owner-private Kernel token for a reachability check but never prints the token or its path; the explicit legacy adapter command remains supported. Launcher Obsidian configuration is persisted in `.hephaestus/efesto-launcher-config.json`, propagated to the child Kernel as `HEPHAESTUS_OBSIDIAN_DIR`, and also read by `server.mjs` as the fallback vault source so reported and written vaults stay identical. The launcher may start only the existing one-click Kernel, may clean only launcher-owned stale records, refuses non-Efesto port conflicts, checks Hermes and Obsidian writeability, only shuts down processes whose pid, owner marker, command fingerprint, start record, and launch nonce still verify the original Efesto process, and leaves daily mission creation to the already-bounded central orb flow.
...
The extension's living pixel forge is a presentation of observable state, not an agent or a source of truth. A pure UI contract maps persisted Agent Hub mission states and explicit manual-capture transitions to idle, queued, working, success, or error scenes. The central forge power surface is one native accessible button containing both the visible orb and its copy, so pointer, Enter, and Space activation reach the same bounded mission controller without invisible overlays or duplicate click listeners. The smith animation cannot imply research when no corresponding state exists, and reduced-motion preferences disable its motion without hiding status text.
