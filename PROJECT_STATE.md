...
- Follow-on branch work: extension consumes Kernel NotificationGateway unread `find:supported:*` receipts as OS chrome.notifications (no new UI). Packaged `efesto-extension.zip` ships `icons/efesto-notification.png` for chrome.notifications (build fail-closes if missing). Watchtower Find OS notify suppressed when Kernel covering receipts exist — covering is state-agnostic so mark-read cannot un-cover before a later watchtower revision; failed/forged-without-finds unchanged.
