...
- Follow-on branch work: extension consumes Kernel NotificationGateway unread `find:supported:*` receipts as OS chrome.notifications (no new UI). Packaged `efesto-extension.zip` ships `icons/efesto-notification.png` for chrome.notifications (build fail-closes if missing). Watchtower Find/forged OS notify suppressed when Kernel covering receipts exist — covering is state-agnostic so mark-read cannot un-cover, and opportunities-empty forged cannot double-fire beside Kernel Find; failed/attention/forged-without-covering unchanged. `presentWatchtowerAviso`, Living Forge `#forge-activity`, and popup `#mission-state` use max(inbox Finds, verificationResults SUPPORT) so empty/partial inbox cannot demote SUPPORT-proven forged missions to Research completed / kind:forged. Watchtower kind:find OS notify click opens Finds (Kernel gateway fallback parity); attention/forged stay on missions. Kernel gateway chrome.notifications copy is generic lock-screen-safe (no Find title/body leak; ARCHITECTURE.md). After OS delivery, markNotificationRead plus a separate state=unread list(limit) advances the unread window (unfiltered list keeps read in-page); covering stays on a state-agnostic list.
- Home forge-state-action (`brainState` forged) fail-closes to focused GoalSurface Kernel SUPPORT (`countMissionKernelSupportedFinds` → mission `findCount` when verificationResults are stripped) — zero SUPPORT on the focused mission is Investigación terminada even when older inbox Finds exist; not Evidence forjada Completado. Home surfaceTitle / empty body also honor focused `findCount` when inbox is empty so Nuevo Goal cannot sit beside Find SUPPORT forjado. Agent Hub KernelWorkspaces `missionWorkspaceBadge`: zero-SUPPORT forged is `research_completed` (unavailable), not healthy forged; ActivityFeed labels `research_completed` as Investigación terminada. Extension `#mission-progress` (`missionJourney`): Forged stage completes only with Kernel SUPPORT (`verificationResults` / GoalSurface `findCount`) — zero-SUPPORT forged leaves Forged pending like completed-without-Evidence. Central Forge Power orb: zero-SUPPORT forged is `research_completed` (neutral; ledger still shown), not green `data-state=completed`. Living Forge `#living-forge` (forge-activity + Goal Surface): zero-SUPPORT forged stays `tone: idle` (not success celebrate), matching orb research_completed → idle. Agent Hub `#mission-state` / mission-card history: zero-SUPPORT forged uses `data-status=research_completed` (dim; not green Completado), matching orb/Living Forge. Home forge-state-action: zero-SUPPORT forged drops `phase-forged` green Completado chrome for `phase-research_completed` (label already Investigación terminada). Shared Goal Truth popup binding (`applyGoalTruthPresentation`) maps forged workState to `completed` / `research_completed` via focused findCount so MutationObserver re-sync cannot overwrite loadAgentHub SUPPORT-gated #mission-state chrome with raw forged. Goal Surface `workState=completed` (completed-without-Evidence) maps to `research_completed` + "Research ended without Evidence" — never green Completado / "No research mission yet" (MutationObserver would undo Agent Hub honesty). Home forge-state-action: GoalSurface `workState=completed` is `Terminada sin Evidence` with `phase-research_completed` (not green `phase-ready` Forja lista). `publicLaunchApproved` remains false.
