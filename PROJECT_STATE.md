...
- Control Center Sites-to-Vercel migration: the ChatGPT-style neural forge, persistent composer, evidence workspaces, Hermes Agent Hub, automation surface, and integration control plane from the published Sites UI now live in `apps/dashboard`. The migrated client rejects non-loopback Kernel URLs before sending the owner token, keeps persistent device reconnection opt-in and off by default, and never turns visual neural activity into Kernel authority.
- Control Center visual-recovery increment: the neural brain now uses explicit intrinsic dimensions and an E2E geometry gate, Hermes has a legible forge identity on dark surfaces, the persistent composer preserves content clearance, conversation search and the owner-settings control work, and research drafts require a separate owner `Execute` action before the Kernel creates a Goal/Hermes mission. The automation surface labels cadence as persisted intent because a general recurring scheduler does not exist.
...
- Fresh local validation on 2026-08-03 on `codex/vercel-github-preview`: `pnpm verify:first-run` passed (108 files / 625 tests), including self-contained dashboard TypeScript/Turbopack deployment regressions and the CI browser-gate contract; typecheck, build, Hermes validators/smokes, altered-replay attack smoke, and Replay Lab API smoke passed; `pnpm audit --prod` reported no known vulnerabilities. The live Kernel returned `200` for health and an authorized local API read, `401` without a token, and `403` for a hostile browser origin. CI now installs Chromium and runs the three Playwright dashboard flows, but this environment could not download the browser binary, so a deployed preview and fresh visual acceptance remain unproven until the remote job runs.
- Fresh local validation on 2026-08-03 on `codex/sites-ui-vercel`: `pnpm verify:first-run` passed (109 files / 628 tests), including the migrated Control Center's loopback-only URL rejection, tab-memory default, token-redaction, disconnect path, build, Hermes validators/smokes, altered-replay attack smoke, and Replay Lab API smoke. `pnpm audit --prod` reported no known vulnerabilities. The three updated desktop/mobile Playwright flows are discoverable, but local execution remains blocked only by the unavailable Chromium binary; remote CI and a Vercel preview must provide the visual acceptance evidence.
- Fresh local validation on 2026-08-04 on `codex/sites-ui-repair`: `pnpm verify:first-run` passed (109 files / 631 tests), including explicit research-draft confirmation, conversation filtering, owner-settings navigation, loopback rejection, rejected-token diagnostics, dashboard typecheck/build, Hermes validators/smokes, altered-replay attack smoke, and Replay Lab API smoke. Playwright now rejects zero-height brain artwork on desktop and mobile; local execution remains blocked by the missing Chromium binary, so a new preview and remote browser gate are still required before visual acceptance.
...
- Current bounded delivery branch: `codex/sites-ui-repair`, based on `codex/sites-ui-vercel`. It repairs the migrated visual shell and closes bounded interaction gaps while preserving the existing Kernel and Vercel preview contracts; no Kernel, agent, memory, or production deployment authority is moved into Vercel.
- Active implementation task: visually verify the repaired Control Center in a new non-production preview and require its strengthened desktop/mobile Playwright gate before review. PR #125 remains design only (memory quarantine and toxic-memory lifecycle; no runtime enforcement).
