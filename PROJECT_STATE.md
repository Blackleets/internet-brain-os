...
- Home forge-state-action (`brainState` forged) fail-closes to focused-mission Kernel SUPPORT Finds (`countMissionKernelSupportedFinds`) — zero SUPPORT on the focused mission is Investigación terminada even when older inbox Finds exist; not Evidence forjada Completado. `publicLaunchApproved` remains false.
