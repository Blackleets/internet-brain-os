...
- Home forge-state-action (`brainState` forged) fail-closes to focused GoalSurface Kernel SUPPORT (`countMissionKernelSupportedFinds` → mission `findCount` when verificationResults are stripped) — zero SUPPORT on the focused mission is Investigación terminada even when older inbox Finds exist; not Evidence forjada Completado. Home surfaceTitle / empty body also honor focused `findCount` when inbox is empty so Nuevo Goal cannot sit beside Find SUPPORT forjado. Agent Hub KernelWorkspaces `missionWorkspaceBadge`: zero-SUPPORT forged is `research_completed` (unavailable), not healthy forged; ActivityFeed labels `research_completed` as Investigación terminada. Extension `#mission-progress` (`missionJourney`): Forged stage completes only with Kernel SUPPORT (`verificationResults` / GoalSurface `findCount`) — zero-SUPPORT forged leaves Forged pending like completed-without-Evidence. Central Forge Power orb: zero-SUPPORT forged is `research_completed` (neutral; ledger still shown), not green `data-state=completed`. `publicLaunchApproved` remains false.
