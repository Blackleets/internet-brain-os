...
- Hephaestus Control Center Phase 1: a presentation-only authenticated loopback dashboard with a truthful Overview of available Kernel read models. Its operator-entered token stays in tab memory by default; the dashboard neither creates a second store nor bypasses Kernel authority.
