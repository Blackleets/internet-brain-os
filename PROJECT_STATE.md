...
- Current bounded delivery branch: `codex/sites-ui-vercel`, based on the validated `codex/vercel-github-preview` delivery branch. It migrates the approved Sites visual experience into the Git-backed dashboard while preserving the existing Kernel and Vercel preview contracts; no Kernel, agent, memory, or production deployment authority is moved into Vercel.
