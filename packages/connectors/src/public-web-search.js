...
exports.PublicWebSearchClient = void 0;
...
class PublicWebSearchClient {
    ...
    async search(query, requestedLimit) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`Public search provider returned HTTP ${response.status}`);
            const contentType = response.headers.get('content-type') ?? '';
            if (!contentType.toLowerCase().includes('html'))
                throw new Error('Public search provider returned a non-HTML response');
...
exports.PublicWebSearchClient = PublicWebSearchClient;
function parseDuckDuckGoHtml(html, limit = 10) {
...
//# sourceMappingURL=public-web-search.js.map