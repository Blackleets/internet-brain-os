...
function fixture(): ReplayLabMemorySafetyViewInput {
  const quarantineResult = evaluateMemoryQuarantineSignals({
    memoryId: 'memory:alpha', lifecycleRevision: 4, state: 'admitted',
    evaluatorVersion: 'quarantine-v1', evaluatedAt: now,
  ...
  const recoveryRecord = new InMemoryMemoryRecoveryReviewRepository().append({
    terminalMemoryId: 'memory:alpha', terminalState: 'revoked', terminalRevision: 6,
    requestId: 'recovery:1', requestedBy: { id: 'recovery:operator', type: 'recovery' },
    reviewer: { id: 'founder:1', type: 'founder' }, policyVersion: 'memory-policy-v1',
    requiresFounderApproval: true, outcome: 'approved_new_candidate',
    replacementCandidateMemoryId: 'memory:alpha:new', reason: 'New verified evidence.', occurredAt: now,
  }).review;
...
describe('Replay Lab memory safety read model', () => {
  it('separates persisted records, human decisions and deterministic interpretations', () => {
    ...
    expect(view.quarantine[0]).toEqual(expect.objectContaining({ basis: 'persisted_record', authority: 'read_only', freshness: 'current' }));
    expect(view.quarantine[0].references).toEqual([{ id: 'evidence:1', kind: 'signal_reference' }]);
    expect(view.recoveryReviews[0]).toEqual(expect.objectContaining({ basis: 'human_decision', authority: 'read_only', freshness: 'stale', replacementCandidateMemoryId: 'memory:alpha:new' }));
    expect(view.recoveryReviews[0].staleReasons).toEqual(['terminal_state_changed', 'terminal_revision_changed', 'policy_version_changed']);
    expect(view.prevention[0]).toEqual(expect.objectContaining({ basis: 'deterministic_projection', authority: 'read_only', freshness: 'current', category: 'altered_replay' }));
    expect(view.prevention[0].references).toEqual([
      { id: 'f1', kind: 'failure' }, { id: 'f2', kind: 'failure' },
      { id: 'event:1', kind: 'persisted_reference' }, { id: 'event:2', kind: 'persisted_reference' },
    ]);
    expect(view.authorityBoundary.status).toBe('read_only');
    expect(view.warnings).toEqual(['1 memory-safety record(s) are historical/stale and must not be treated as current authorization.']);
  });
