...
describe('internal Efesto release package', () => {
  ...
  it('requires one immutable candidate plus clean-install, real-web, memory-safety, replay and failure UAT before launch', async () => {
    const release = JSON.parse(await text('INTERNAL_RELEASE.json'));
    ...
    expect(uat).toContain('UAT-4 — persistence, memory safety and replay');
