...
function isClaimable(mission, now) {
  ...
  return mission.status === 'running'
    ...
    && Number(mission.attempt ?? 0) < MAX_ATTEMPTS;
}
...
function validateFinding(value, index) {
  ...
  return {
    url: parsed.href,
    title: clean(value.title, 240, 'title'),
    text: clean(value.text, 20_000, 'text'),
    summary: optional(value.summary, 500, 'summary'),
    discoveredAt: optionalDate(value.discoveredAt),
  };
}
