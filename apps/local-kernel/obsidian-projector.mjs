...
function renderEvidence(item, caseRecord) {
  return [
    ...
    ...(item.aiSummary ? [
      ...
      '',
      ...(item.aiSummary.hypotheses.length ? item.aiSummary.hypotheses.map((value) => `- Hypothesis: ${value}`) : ['- None generated.']),
      '',
      '### Model limitations',
      '',
      ...(item.aiSummary.limitations.length ? item.aiSummary.limitations.map((value) => `- ${value}`) : ['- No model limitations supplied.']),
      '',
...
function renderReport(caseRecord, evidence) {
  ...
  return [
    ...
    '',
    ...(evidence.length
      ? evidence.map((item) => `- [[Evidence/${safeId(item.id)}|${item.aiSummary?.summary ?? item.summary ?? item.id}]] — confidence ${item.confidence}`)
      : ['No Evidence captured yet.']),
    '',
    '## Limitations',
    '',
