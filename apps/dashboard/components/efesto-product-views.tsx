    ...
    <div className="forge-scroll">
      {chatMode ? messages.length ? <section className="forge-thread" aria-label="Mensajes">
        ...
        <ol>
          ...
          <li><b>3</b><span><strong>Forjar evidencia y hallazgos</strong><small>Solo se muestran resultados respaldados por contratos verificables.</small></span></li>
        </ol>
        ...
        <small>EFESTO · MISIÓN CONTROLADA</small>
        <h1>Define el resultado</h1>
        <p>Efesto prepara el plan. Tú decides si se ejecuta.</p>
        <div className="forge-empty-meta"><span><ShieldCheck /> Controlado por Kernel</span><span><i /> Confirmación humana</span></div>
      </section>}
...
function ComposerForm({ input, chatMode, chatAvailable, chatPending, submitDisabled, suggestions, onSuggestion, onInputChange, onSubmit, onStopChat, onOpenModels, modelLabel, providers, selectedProviderId, selectedModel, connected, onSelectModel, onOpenSettings }: {
  ...
  return <div className={'forge-composer-zone ' + (suggestions.length ? 'has-suggestions' : '')}>
      ...
      <footer>
        <div className="forge-composer-context">
          {chatMode ? <ModelSelector
            ...
            onOpenModels={onOpenModels}
            onOpenSettings={onOpenSettings}
          /> : <span className="forge-kernel-gate"><ShieldCheck /> Controlado por Kernel</span>}
        </div>
        <span className="forge-shortcut">{chatMode ? 'Enter para enviar' : 'Enter para preparar'}</span>
    ...
    <p className="forge-composer-hint" id="forge-composer-note">{chatMode ? 'La conversación permanece separada de la evidencia y la memoria.' : 'Preparar no ejecuta ninguna acción externa.'}</p>
  </div>;
}
...
export function MissionsView({ snapshot, onNew }: { snapshot?: OverviewSnapshot; onNew: () => void }) {
  ...
  return <Workspace icon={Target} eyebrow="Goal → Misión → Evidencia" title="Misiones" copy="Ejecuciones reales reclamadas por agentes bajo autoridad del Kernel." action={<button type="button" className="primary-action" onClick={onNew}><Target /> Nuevo Goal</button>}>
    {!snapshot ? <Empty icon={CircleOff} title="Kernel sin conexión" copy="Conecta el Kernel para leer el registro real de misiones." /> : missions.length === 0 ? <Empty icon={Target} title="No hay misiones" copy="Crea un Goal; no simulamos ejecuciones vacías." /> : <div className="record-list">{missions.map((mission) => <article key={mission.id}><div className="record-icon"><Target /></div><div><strong>{goalName(mission.goalId)}</strong><small>{mission.id} · intento {mission.attempt ?? 0}</small></div><StatePill state={mission.executionPhase ?? mission.status} /></article>)}</div>}
  </Workspace>;
}
...
export function FindsView({ opportunities, connected, onFeedback }: { opportunities: OpportunitySummary[]; connected: boolean; onFeedback: (id: string, signal: 'useful' | 'saved' | 'dismissed' | 'not_interested') => void }) {
  return <Workspace icon={Sparkles} eyebrow="Inteligencia de oportunidades" title="Hallazgos" copy="Hallazgos priorizados por el Kernel. Tu feedback cambia preferencias, no reescribe la evidencia objetiva.">
    {!connected ? <Empty icon={CircleOff} title="Kernel sin conexión" copy="Conecta el Kernel para cargar hallazgos reales." /> : opportunities.length === 0 ? <Empty icon={Search} title="Aún no hay hallazgos" copy="Ejecuta un Goal público y los resultados promovidos aparecerán aquí." /> : <div className="find-grid">{opportunities.map((item) => <article key={item.id}><header><span>{item.categoryLabel}</span><StatePill state={item.status} /></header><h2>{item.title}</h2><p>{item.sourceHost} · relevancia {formatRelevance(item.relevance)}</p><div className="find-next"><small>Siguiente paso</small><strong>{item.nextAction}</strong></div><div className="find-actions"><button type="button" onClick={() => onFeedback(item.id, 'useful')}><Check /> Útil</button><button type="button" onClick={() => onFeedback(item.id, 'saved')}><History /> Guardar</button><button type="button" onClick={() => onFeedback(item.id, 'dismissed')}><X /> Descartar</button></div></article>)}</div>}
  </Workspace>;
}
...
export function EvidenceView({ cases, selectedId, detail, loadingId, connected, onOpen }: { cases: CaseSummary[]; selectedId: string; detail?: CaseDetail; loadingId: string; connected: boolean; onOpen: (record: CaseSummary) => void }) {
  return <Workspace icon={ShieldCheck} eyebrow="Casos · Fuentes · Procedencia" title="Evidencia" copy="Inspecciona recibos persistidos. Cada URL visible viene del Kernel.">
    {!connected ? <Empty icon={CircleOff} title="Kernel sin conexión" copy="Conecta el Kernel para leer casos y evidencia." /> : <div className="evidence-layout"><section className="case-list"><header><span>Casos</span><b>{cases.length}</b></header>{cases.length ? cases.map((record) => <button type="button" key={record.id} className={selectedId === record.id ? 'active' : ''} onClick={() => onOpen(record)}><FileSearch /><span><strong>{record.title}</strong><small>{record.status} · {record.id}</small></span><ChevronRight /></button>) : <Empty icon={FileSearch} title="Sin casos" copy="El Kernel devolvió una colección vacía." />}</section><section className="evidence-detail">{!selectedId ? <Empty icon={ShieldCheck} title="Selecciona un caso" copy="Aquí aparecerán sus fuentes y recibos reales." /> : loadingId === selectedId ? <Empty icon={Activity} title="Leyendo evidencia" copy="Esperando respuesta del Kernel." /> : !detail ? <Empty icon={CircleOff} title="Evidencia no disponible" copy="No inventamos una proyección cuando el endpoint no responde." /> : detail.evidence.length === 0 ? <Empty icon={ShieldCheck} title="Caso sin evidencia publicada" copy="El caso existe, pero todavía no tiene recibos visibles." /> : <div className="evidence-receipts"><header><small>CASO</small><h2>{recordTitle(detail.case, selectedId)}</h2></header>{detail.evidence.map((item, index) => <article key={item.id ?? index}><ShieldCheck /><div><strong>{item.summary ?? item.id ?? `Evidencia ${index + 1}`}</strong><small>{typeof item.confidence === 'number' ? `Confianza ${Math.round(item.confidence * 100)}%` : 'Confianza no publicada'}{item.capturedAt ? ` · ${formatDate(item.capturedAt)}` : ''}</small>{item.tags?.length ? <p>{item.tags.join(' · ')}</p> : null}</div>{item.sourceUrl ? <a href={item.sourceUrl} target="_blank" rel="noreferrer">Abrir fuente <ExternalLink /></a> : <span className="source-missing">Sin URL publicada</span>}</article>)}</div>}</section></div>}
  </Workspace>;
}
...
export function ModelsView({ providers, selectedProviderId, selectedModel, modelForge, connected, onSelect, onAdd }: { providers: Provider[]; selectedProviderId: string; selectedModel: string; modelForge?: ModelForgeSummary; connected: boolean; onSelect: (providerId: string, model: string) => void; onAdd: (event: FormEvent<HTMLFormElement>) => void }) {
  return <Workspace icon={BrainCircuit} eyebrow="Forja de modelos · Proveedores de chat" title="Modelos" copy="Elige modelos configurados de verdad. Ningún modelo recibe autoridad sobre la memoria.">
    {!connected ? <Empty icon={CircleOff} title="Kernel sin conexión" copy="Conecta el Kernel para leer proveedores y la forja de modelos." /> : <div className="models-layout"><section className="model-list"><header><span>Disponibles</span><b>{providers.reduce((total, provider) => total + provider.models.length, 0)}</b></header>{providers.length ? providers.flatMap((provider) => provider.models.map((model) => <button type="button" className={provider.id === selectedProviderId && model === selectedModel ? 'active' : ''} key={`${provider.id}:${model}`} onClick={() => onSelect(provider.id, model)}><BrainCircuit /><span><strong>{model}</strong><small>{provider.label} · {provider.managedBy === 'environment' ? 'entorno privado' : 'Kernel local'}</small></span>{provider.id === selectedProviderId && model === selectedModel ? <Check /> : <ChevronRight />}</button>)) : <Empty icon={BrainCircuit} title="Sin modelos configurados" copy="Añade un proveedor real a la derecha." />}{modelForge ? <div className="forge-hardware"><small>FORJA DE MODELOS</small><strong>{modelForge.runtime === 'available' ? `${modelForge.hardware.tier} · ${modelForge.hardware.ramGiB} GiB RAM` : 'Runtime no detectado'}</strong><span>Recomendado: {modelForge.recommended}</span></div> : null}</section><form className="provider-form" onSubmit={onAdd}><header><Plug /><div><small>PROPIEDAD DEL KERNEL</small><strong>Añadir proveedor</strong></div></header><label>Tipo<select name="type" defaultValue="openai-compatible"><option value="openai-compatible">OpenAI compatible</option><option value="ollama">Ollama local</option></select></label><label>Nombre<input name="label" required placeholder="OpenRouter, Groq, Ollama…" /></label><label>Endpoint<input name="baseUrl" type="url" required placeholder="https://api.example.com/v1" /></label><label>Modelos<input name="models" required placeholder="modelo-1, modelo-2" /></label><label>Credencial<input name="apiKey" type="password" autoComplete="off" placeholder="Opcional para Ollama" /></label><button type="submit" className="primary-action"><Plug /> Guardar en Kernel</button><p><ShieldCheck /> La credencial no se devuelve al navegador después de guardarla.</p></form></div>}
  </Workspace>;
}
...
export function AgentsView({ snapshot, onSettings, onNewGoal }: { snapshot?: OverviewSnapshot; onSettings: () => void; onNewGoal: () => void }) {
  ...
  return <Workspace icon={Bot} eyebrow="Ejecución controlada" title="Agentes" copy="Hermes y futuros agentes pueden ejecutar herramientas; el Kernel conserva la autoridad." action={<button type="button" className="secondary-action" onClick={onSettings}><Settings /> Configurar</button>}>
    <div className="agent-hero"><div className="agent-mark"><Bot /></div><div><small>NOUS RESEARCH</small><h2>Hermes Agent</h2><p>Descubrimiento y ejecución acotada. Sus hallazgos deben regresar por el puente autenticado.</p></div><StatePill state={hermes === 'ready' ? 'ready' : hermes ?? 'offline'} /></div><div className="agent-contract"><span><b>Kernel</b>{snapshot?.readiness.kernel ?? 'offline'}</span><span><b>Hermes</b>{hermes ?? 'sin diagnóstico'}</span><span><b>Misiones</b>{snapshot?.missions.length ?? 0}</span><span><b>Autoridad</b>Solo Kernel</span></div><button type="button" className="primary-action" onClick={onNewGoal}><Target /> Preparar una misión</button>
  </Workspace>;
}
...
export function AutomationsView({ missions, connected, onNewGoal }: { missions: MissionSummary[]; connected: boolean; onNewGoal: () => void }) {
  ...
  return <Workspace icon={Workflow} eyebrow="Intenciones persistidas" title="Automatizaciones" copy="Solo mostramos automatizaciones que el Kernel puede demostrar. No hay interruptores ni próximas ejecuciones inventadas." action={<button type="button" className="primary-action" onClick={onNewGoal}><Target /> Nuevo Goal</button>}>
...
export function SettingsView({ connected, connecting, rememberSession, snapshot, onConnect, onDisconnect, onRefresh }: { connected: boolean; connecting: boolean; rememberSession: boolean; snapshot?: OverviewSnapshot; onConnect: (event: FormEvent<HTMLFormElement>) => void; onDisconnect: () => void; onRefresh: () => void }) {
  ...
  return <Workspace icon={Settings} eyebrow="Panel de control del propietario" title="Ajustes" copy="Conexión local, preparación y recuperación. Los secretos no se imprimen.">
    <div className="settings-layout"><form className="connection-card" onSubmit={onConnect}><header><span className={`kernel-dot ${connected ? 'online' : 'offline'}`} /><div><small>DISPOSITIVO ACTUAL</small><strong>{connected ? 'Autorizado' : 'Conectar Kernel'}</strong></div></header>{!connected ? <><label>URL del Kernel<input name="baseUrl" aria-label="URL del Kernel" type="url" defaultValue="http://127.0.0.1:4000" required /></label><label>Token privado<input name="token" aria-label="Token privado" type="password" autoComplete="off" placeholder="Opcional si usas emparejamiento" /></label><label>Código de emparejamiento<input name="pairingCode" aria-label="Código de emparejamiento" inputMode="text" autoComplete="off" placeholder="ABCD-1234" /></label><label className="remember-row"><input name="rememberSession" type="checkbox" defaultChecked={rememberSession} /><span><strong>Recordar durante esta sesión</strong><small>Solo se conserva mientras esta sesión está abierta.</small></span></label><div className="connection-actions"><button type="submit" name="action" value="pair" className="primary-action" disabled={connecting}>{connecting ? 'Emparejando…' : 'Emparejar con código'}</button><button type="submit" name="action" value="token" className="secondary-action" disabled={connecting}>Autorizar dispositivo</button></div></> : <div className="connection-actions"><button type="button" className="primary-action" onClick={onRefresh}><RefreshCw /> Comprobar ahora</button><button type="button" className="danger-action" onClick={onDisconnect}>Desconectar</button></div>}</form><section className="readiness-card"><header><ShieldCheck /><div><small>ESTADO REAL</small><strong>{statusLabel(bootstrap?.overall ?? (connected ? 'connected' : 'offline'))}</strong></div></header><ReadinessRow label="Kernel" value={bootstrap?.kernel ?? (connected ? 'ready' : 'offline')} ready={connected} /><ReadinessRow label="Hermes" value={bootstrap?.hermes ?? 'sin diagnóstico'} ready={bootstrap?.hermes === 'ready'} /><ReadinessRow label="Obsidian" value={bootstrap?.obsidian ?? 'sin configurar'} ready={bootstrap?.obsidian === 'ready'} /><ReadinessRow label="Extensión" value={bootstrap?.pairing ?? 'sin emparejar'} ready={bootstrap?.pairing === 'paired'} /><a href="http://127.0.0.1:4000/replay-lab" target="_blank" rel="noreferrer">Abrir Replay Lab <ExternalLink /></a></section></div>
  </Workspace>;
}
...
function StatePill({ state }: { state: string }) { const tone = ['ready', 'completed', 'forged', 'available', 'new'].includes(state) ? 'good' : ['failed', 'invalid'].includes(state) ? 'bad' : ['running', 'investigating', 'verifying', 'queued', 'waiting_for_agent'].includes(state) ? 'working' : 'neutral'; return <span className={`state-pill ${tone}`}><i />{statusLabel(state)}</span>; }
function ReadinessRow({ label, value, ready }: { label: string; value: string; ready: boolean }) { return <div className="readiness-row"><span>{label}</span><strong className={ready ? 'ready' : ''}><i />{statusLabel(value)}</strong></div>; }
function statusLabel(value: string) { return ({ available: 'disponible', completed: 'completada', configured: 'configurado', connected: 'conectado', failed: 'fallida', forged: 'forjada', invalid: 'inválida', new: 'nueva', offline: 'desconectado', paired: 'emparejado', queued: 'en cola', ready: 'listo', running: 'en curso', verifying: 'verificando', investigating: 'investigando', waiting_for_agent: 'esperando agente', unconfigured: 'sin configurar', checking: 'comprobando' } as Record<string, string>)[value] ?? value.replaceAll('_', ' '); }
export function brainState(phase: BrainPhase) { if (phase === 'thinking') return { label: 'Conversando', detail: 'Modelo transmitiendo' }; if (phase === 'investigating') return { label: 'Investigando', detail: 'Hermes ejecutando una misión' }; if (phase === 'verifying') return { label: 'Verificando evidencia', detail: 'Kernel aplicando controles' }; if (phase === 'queued') return { label: 'Misión preparada', detail: 'Esperando agente' }; if (phase === 'forged') return { label: 'Evidencia forjada', detail: 'Resultado persistido' }; if (phase === 'failed') return { label: 'Atención requerida', detail: 'La última misión falló' }; if (phase === 'ready') return { label: 'Forja lista', detail: 'Lista para un nuevo Goal' }; return { label: 'Modo local desconectado', detail: 'Sin actividad simulada' }; }
