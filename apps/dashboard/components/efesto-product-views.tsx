...
export function IntegrationsView({ catalog, connected, onNavigate, onRefresh, onBack }: { catalog?: IntegrationCatalog; connected: boolean; onNavigate: (action: IntegrationAction) => void; onRefresh: () => void; onBack: () => void }) {
  ...
  const pendingCount = integrations.length - readyCount;
  ...
  return <section className="workspace forge-integrations-workspace">
    ...
    <div className="forge-integrations-intro">
      <div className="forge-integrations-intro-copy"><small>{t('integrations.eyebrow')}</small><h1>{t('integrations.title')}</h1><p>{t('integrations.copy')}</p></div>
      {catalog ? <div className="forge-integrations-summary" aria-label={t('integrations.summaryAria')}>
        <span><strong>{readyCount}</strong><small>{t('integrations.summary.ready')}</small></span>
        <span><strong>{pendingCount}</strong><small>{t('integrations.summary.pending')}</small></span>
        <span className="is-boundary"><ShieldCheck /><small>{t('integrations.summary.readOnly')}</small></span>
      </div> : null}
    </div>
    {!connected ? <Empty icon={CircleOff} title={t('missions.offlineTitle')} copy={t('integrations.offlineCopy')} /> : !catalog ? <Empty icon={Plug} title={t('integrations.unavailableTitle')} copy={t('integrations.unavailableCopy')} /> : <>
      <div className="forge-addon-icon-strip" aria-label={t('integrations.topAria')}>
        <span className="forge-addon-icon-strip-label">{t('integrations.quickNav')}</span>
        {integrations.map((integration) => <button type="button" key={integration.id} className={`forge-addon-icon-chip status-${integration.status}`} onClick={() => scrollToIntegration(integration.id)} aria-label={t(`integrations.${integration.id}.name`)} title={t(`integrations.${integration.id}.name`)}>
          <span className="forge-addon-icon-chip-mark"><IntegrationMark id={integration.id} /></span>
          <span className="forge-addon-icon-chip-copy"><strong>{t(`integrations.${integration.id}.name`)}</strong><small>{statusLabel(integration.status, t)}</small></span>
        </button>)}
      </div>
      <div className="forge-integrations-trustline" role="note">
        <span className="forge-integrations-trustline-mark"><ShieldCheck /></span>
        <span className="forge-integrations-trustline-copy"><strong>{t('integrations.boundaryTitle')}</strong><small>{t('integrations.boundaryCopy')}</small></span>
        <button type="button" onClick={onRefresh} aria-label={t('integrations.refresh')}><RefreshCw /></button>
      </div>
