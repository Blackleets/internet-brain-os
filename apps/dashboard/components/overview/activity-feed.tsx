...
type ActivityFeedProps = { activity: OverviewActivity[]; unavailable: boolean };
...
export function ActivityFeed({ activity, unavailable }: ActivityFeedProps) {
  return <Panel title="Actividad reciente" eyebrow="Registros persistidos" className="overview-activity">{unavailable ? <p className="empty-state" role="status">Datos temporalmente no disponibles</p> : activity.length === 0 ? <p className="empty-state">No hay actividad persistida para mostrar.</p> : <ol className="activity-list">{activity.map((entry) => (
